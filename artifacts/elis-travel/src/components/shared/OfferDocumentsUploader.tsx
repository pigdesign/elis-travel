...
interface OfferDocumentsUploaderProps {
  offerId: string;
}

export function OfferDocumentsUploader({ offerId }: OfferDocumentsUploaderProps) {
  const fileInputRef = useRef<HTMLInputElement>(null);
  const [uploadError, setUploadError] = useState<string | null>(null);
  const [uploading, setUploading] = useState(false);

  const { data: documents = [], isLoading } = useGetOfferDocuments(offerId);
  const { mutateAsync: addDocument } = useAddOfferDocument(offerId);
  const { mutate: deleteDocument, isPending: isDeleting } = useDeleteOfferDocument(offerId);
  ...
  async function handleFile(file: File) {
    ...
    try {
      ...
      await addDocument({ documentUrl: publicUrl, fileName: file.name });
    } catch (err) {
    ...
    <div className="space-y-3">
      {documents.length > 0 && (
        <ul className="space-y-2">
          {documents.map((doc: OfferDocument) => (
            <li
              key={doc.id}
