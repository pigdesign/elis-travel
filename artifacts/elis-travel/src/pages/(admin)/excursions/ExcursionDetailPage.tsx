  ...
  const reportRows = activeBookings
    .map((b) => ({
      ...
      pickup: b.pickupPointId
        ...
        : (excursionPickupPoints && excursionPickupPoints.length > 0 ? "Punti diversi" : ""),
      adults: b.adults,
      children: b.children,
                ...
                <div className="pl-3 space-y-1">
                  {exc.otherCosts.map((oc, i) => (
                    <div key={i} className="flex justify-between text-xs text-muted-foreground/70">
                      <span className="truncate pr-2">{oc.name?.trim() || "Voce"}</span>
                      <span className="shrink-0">– {formatEur(Number(oc.price) || 0)}</span>
                    </div>
                  ))}
                </div>
              )}
